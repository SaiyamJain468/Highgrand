...
export default async function ResellerLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-brand-black overflow-hidden pt-16">
      {/* Sidebar - Only show if not pending */}
      {!isPending && (
        ...
        {isPending ? (
          <div className="h-full flex flex-col relative">
            <div className="absolute top-4 right-4 z-50">
               <form action="/api/auth/signout" method="POST">
                <button type="submit" className="flex items-center gap-2 px-4 py-2 border border-brand-border text-brand-muted hover:text-brand-white hover:bg-brand-surface1 transition-colors rounded-sm text-[12px] font-medium uppercase tracking-widest">
